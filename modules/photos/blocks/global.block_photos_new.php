...
if( !nv_function_exists( 'nv_block_photos_new' ) )
{
	function nv_block_config_photos_new( $module, $data_block, $lang_block )
	{
		...
		$html .= '<div class="col-sm-9"><input type="text" class="form-control w200" name="config_title_length" size="5" value="' . $data_block['title_length'] . '"/></div>';
		$html .= '</div>';
		$html .= '<div class="form-group">';
		$html .= '<label class="control-label col-sm-6">' . $lang_block['title_length'] . '</label>';
		$html .= '<div class="col-sm-9"><input type="text" class="form-control w200" name="config_title_length" size="5" value="' . $data_block['title_length'] . '"/></div>';
		...
		$html .= '<div class="form-group">';
		$html .= '<label class="control-label col-sm-6">' . $lang_block['slide_mode'] . '</label>';
		$html .= '<div class="col-sm-9"><input type="checkbox" class="form-control" name="config_slide_mode" value="1" ' . ($data_block['slide_mode'] == 1 ? 'checked="checked"' : '') . '"/></div>';
		$html .= '</div>';
		$html .= '<div class="form-group">';
        $html .= '	<label class="control-label col-sm-6">' . $lang_block['hometext'] . ':</label>';
        $html .= '	<div class="col-sm-18"><input type="text" class="form-control" name="config_hometext" value="' . $data_block['hometext'] . '"/></div>';
        $html .= '</div>';
	...
	function nv_block_config_photos_new_submit( $module, $lang_block )
	{
		...
		$return['config']['hometext'] = $nv_Request->get_string('config_hometext', 'post', 0);
		return $return;
	}
	...
	function nv_block_photos_new( $block_config )
	{
		...
		$db->sqlreset( )->select( '*' )->from( NV_PREFIXLANG . '_' . $mod_data . '_rows' )->where( 'status= 1 ' )->order( ' rand() DESC' )->limit( $block_config['numrow'] );
		...
		if( !empty( $list ) )
		{
			...
			$xtpl->assign('BLOCK_HOME_TEXT', $block_config['hometext']);
			foreach( $list as $album )
			{
				$album['name'] = nv_clean60( $album['name'], $block_config['title_length'] );
				$album['link'] = NV_BASE_SITEURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&amp;' . NV_NAME_VARIABLE . '=' . $module . '&amp;' . NV_OP_VARIABLE . '=' .  $album['alias'] . '-' . $album['album_id'] . '/' . $album['row_id'] . $global_config['rewrite_exturl'];
