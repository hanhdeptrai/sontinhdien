...
if (!nv_function_exists('nvb_quytrinh')) {
    /**
     * @param string $module
     ...
     * @return string
     */
    function nvb_config_quytrinh($module, $data_block, $lang_block)
    {
     ...
     * @return number
     */
    function nvb_config_quytrinh_submit($module, $lang_block)
    {
        global $nv_Request;
     ...
     * @return string
     */
    function nvb_quytrinh($block_config)
    {
        ...
        if (isset($site_mods[$mod_name])) {
			...
			list ($titlebid, $descriptionbid) = $db_slave->query('SELECT title, description FROM ' . NV_PREFIXLANG . '_' . $mod_data . '_slider where  status=1 AND   bid='.$block_config['blockid'])->fetch(3);
            ...
            if (file_exists(NV_ROOTDIR . '/themes/' . $global_config['module_theme'] . '/modules/' . $mod_file . '/global.quytrinh.tpl')) {
                $block_theme = $global_config['module_theme'];
            } elseif (file_exists(NV_ROOTDIR . '/themes/' . $global_config['site_theme'] . '/modules/' . $mod_file . '/global.quytrinh.tpl')) {
                $block_theme = $global_config['site_theme'];
            } else {
                $block_theme = 'default';
            }

            $xtpl = new XTemplate('global.quytrinh.tpl', NV_ROOTDIR . '/themes/' . $block_theme . '/modules/' . $mod_file);
...
if (defined('NV_SYSTEM')) {
    $content = nvb_quytrinh($block_config);
}
