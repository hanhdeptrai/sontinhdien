...
if( !nv_function_exists( 'nv_block_category_album' ) )
{
	function nv_block_config_category_album( $module, $data_block, $lang_block )
	{
		...
		$html .= '<tr>';
		$html .= '<td>' . $lang_block['hometext'] . '</td>';
		$html .= '<td><input type="text" class="form-control w200" name="config_hometext" size="5" value="' . $data_block['hometext'] . '"/></td>';
		$html .= '</tr>';
	...
	function nv_block_config_category_album_submit( $module, $lang_block )
	{
		...
		$return['config']['hometext'] = $nv_Request->get_title( 'config_hometext', 'post', 0 );
	...
	function nv_block_category_album( $block_config )
	{
		...
		if( !empty( $list ) )
		{
			...
			$xtpl->assign( 'TITILE_BLOCK', $block_config['title'] );
			...
			foreach( $list as $album )
			{
				...
				$album['link'] = NV_BASE_SITEURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&amp;' . NV_NAME_VARIABLE . '=' . $module . '&amp;' . NV_OP_VARIABLE . '=' . $album['alias'] . '-' . $album['album_id'];
