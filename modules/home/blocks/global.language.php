...
if (!nv_function_exists('nvb_home_language')) {
    /**
     * @param string $module
     ...
     * @return string
     */
    function nvb_config_home_language($module, $data_block, $lang_block)
    {
     ...
     * @return number
     */
    function nvb_config_home_language_submit($module, $lang_block)
    {
        global $nv_Request;
     ...
     * @return string
     */
    function nvb_home_language($block_config)
    {
        ...
        if (isset($site_mods[$mod_name])) {
            ...
            $sql = 'SELECT title, title_extra, title_link, description, link, image, image_mobile, addtime, weight, status FROM ' . NV_PREFIXLANG . '_' . $mod_data . '_slider_rows where bid='.$block_config['blockid'].' AND status=1 order by weight DESC';
            $list = $nv_Cache->db($sql, '', $mod_name);

            if (file_exists(NV_ROOTDIR . '/themes/' . $global_config['module_theme'] . '/modules/' . $mod_file . '/global.language.tpl')) {
                $block_theme = $global_config['module_theme'];
            } elseif (file_exists(NV_ROOTDIR . '/themes/' . $global_config['site_theme'] . '/modules/' . $mod_file . '/global.language.tpl')) {
                $block_theme = $global_config['site_theme'];
            } else {
                $block_theme = 'default';
            }

            $xtpl = new XTemplate('global.language.tpl', NV_ROOTDIR . '/themes/' . $block_theme . '/modules/' . $mod_file);
...
if (defined('NV_SYSTEM')) {
    $content = nvb_home_language($block_config);
}
