...
function nv_site_theme($contents, $full = true)
{
	...
	$xtpl->assign('NV_PHONE', $global_config['site_phone']);
	$xtpl->assign('NV_EMAIL', $global_config['site_email']);
...
global $nv_Cache;
    ...
    $home_list = $nv_Cache->db($sql, '', $module_name);
    $home_config = array();
    foreach ($home_list as $values) {
      $home_config[$values['config_name']] = $values['config_value'];

  }
  $xtpl->assign('HOME', $home_config);
   if (isset($site_mods['feeds'])) {
            ...
            $xtpl->parse('main.header_feeds');
        }
  if(!empty($home_config['company_email'])){   $xtpl->parse('main.header_email'); }
  if(!empty($home_config['facebook'])){   $xtpl->parse('main.facebook');$xtpl->parse('main.header_facebook'); }
  if(!empty($home_config['youtube'])){  $xtpl->parse('main.youtube');$xtpl->parse('main.header_youtube'); }
  if(!empty($home_config['instagram'])){  $xtpl->parse('main.instagram'); $xtpl->parse('main.header_instagram'); }
  if(!empty($home_config['twitter'])){  $xtpl->parse('main.twitter');$xtpl->parse('main.header_twitter'); }
  if(!empty($home_config['video'])){ $xtpl->parse('main.video'); }
  if(!empty($home_config['logobocongthuong'])){  $xtpl->parse('main.logobocongthuong'); }
  if (! empty($home_config['company_hotline'])) {
    $nums = array_map('trim', explode('|', nv_unhtmlspecialchars($home_config['company_hotline'])));
