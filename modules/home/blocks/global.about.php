...
if (!nv_function_exists('nvb_about')) {
    /**
     * @param string $module
     ...
     * @return string
     */
    function nvb_config_about($module, $data_block, $lang_block)
    {
     ...
     * @return number
     */
    function nvb_config_about_submit($module, $lang_block)
    {
        global $nv_Request;
     ...
     * @return string
     */
    function nvb_about($block_config)
    {
        ...
        if (isset($site_mods[$mod_name])) {
			...
			list ($titlebid, $descriptionbid,$image,$image2) = $db_slave->query('SELECT title, description,image,image2 FROM ' . NV_PREFIXLANG . '_' . $mod_data . '_slider where  status=1 AND   bid='.$block_config['blockid'])->fetch(3);


            $sql = 'SELECT title, title_extra, title_link, description, link, image, image_mobile, addtime, weight, status FROM ' . NV_PREFIXLANG . '_' . $mod_data . '_slider_rows where status=1 and  bid='.$block_config['blockid'];
            $list = $nv_Cache->db($sql, '', $mod_name);

            if (file_exists(NV_ROOTDIR . '/themes/' . $global_config['module_theme'] . '/modules/' . $mod_file . '/global.about.tpl')) {
                $block_theme = $global_config['module_theme'];
            } elseif (file_exists(NV_ROOTDIR . '/themes/' . $global_config['site_theme'] . '/modules/' . $mod_file . '/global.about.tpl')) {
                $block_theme = $global_config['site_theme'];
            } else {
                $block_theme = 'default';
            }

            $xtpl = new XTemplate('global.about.tpl', NV_ROOTDIR . '/themes/' . $block_theme . '/modules/' . $mod_file);
            $xtpl->assign('NV_BASE_SITEURL', NV_BASE_SITEURL);
            $xtpl->assign('TEMPLATE', $block_theme); 
	
			$xtpl->assign('BLOCK_TITLE', $titlebid);
			$xtpl->assign('BLOCK_HOME_TEXT', $descriptionbid);
			$xtpl->assign('IMG', NV_BASE_SITEURL . NV_UPLOADS_DIR . '/' . $mod_upload . '/' . $image);
			$xtpl->assign('IMG2', NV_BASE_SITEURL . NV_UPLOADS_DIR . '/' . $mod_upload . '/' . $image2);
			if (!empty($image)) {
                    $xtpl->parse('main.img1');
                }
			if (!empty($image2)) {
            ...
            foreach ($list as $row) {
              ...
              if($i==2){$xtpl->assign('COLOR', 'about_item_color');}
			  else{$xtpl->assign('COLOR', 'about_item_title');}
...
if (defined('NV_SYSTEM')) {
    $content = nvb_about($block_config);
}
