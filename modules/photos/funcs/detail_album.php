...
if( nv_user_in_groups( $global_photo_album[$album_id]['groups_view'] ) )
{
	...
	// album cung chu de
	$sql = 'SELECT a.album_id, a.name, a.alias,  a.description, a.num_photo, a.date_added, a.viewed, r.file, r.thumb FROM ' . TABLE_PHOTO_NAME . '_album a
		LEFT JOIN  ' . TABLE_PHOTO_NAME . '_rows r ON ( a.album_id = r.album_id )
		WHERE a.status= 1 AND  r.defaults = 1 AND a.album_id <> ' . $album['album_id'] . '
	...
	{
		$item['link'] = nv_url_rewrite( NV_BASE_SITEURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&amp;' . NV_NAME_VARIABLE . '=' . $module_name . '&amp;' . NV_OP_VARIABLE . '=' . $item['alias'] . '-' . $item['album_id'] , true );
