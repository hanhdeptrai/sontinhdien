...
if (!nv_function_exists('nvb_home_doitac')) {
    /**
     * @param string $module
     ...
     * @return string
     */
    function nvb_config_home_doitac($module, $data_block, $lang_block)
    {
     ...
     * @return number
     */
    function nvb_config_home_doitac_submit($module, $lang_block)
    {
        global $nv_Request;
     ...
     * @return string
     */
    function nvb_home_doitac($block_config)
    {
        ...
        if (isset($site_mods[$mod_name])) {
            ...
            if (file_exists(NV_ROOTDIR . '/themes/' . $global_config['module_theme'] . '/modules/' . $mod_file . '/global.doitac.tpl')) {
                $block_theme = $global_config['module_theme'];
            } elseif (file_exists(NV_ROOTDIR . '/themes/' . $global_config['site_theme'] . '/modules/' . $mod_file . '/global.doitac.tpl')) {
                $block_theme = $global_config['site_theme'];
            } else {
                $block_theme = 'default';
            }

            $xtpl = new XTemplate('global.doitac.tpl', NV_ROOTDIR . '/themes/' . $block_theme . '/modules/' . $mod_file);
...
if (defined('NV_SYSTEM')) {
    $content = nvb_home_doitac($block_config);
}
