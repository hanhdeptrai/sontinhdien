...
if ($nv_Request->get_int('save', 'post') == '1') {
    ...
    $row['video'] = $nv_Request->get_title('video', 'post', ''); 
    ...
    if (empty($row['title'])) {
        ...
        if ($id and !$copy) {
            $_sql = 'UPDATE ' . NV_PREFIXLANG . '_' . $module_data . ' SET
                title = :title, alias = :alias, video = :video,  image = :image, imagealt = :imagealt,
            ...
            $_sql = 'INSERT INTO ' . NV_PREFIXLANG . '_' . $module_data . ' (
                title, alias, video,image, imagealt, imageposition, description, bodytext, keywords,
                socialbutton, activecomm, layout_func, weight,admin_id, add_time, edit_time, status,hot_post
            ) VALUES (
                :title, :alias, :video,:image, :imagealt, :imageposition, :description, :bodytext,
        ...
        try {
            ...
            $sth->bindParam(':video', $row['video'], PDO::PARAM_STR);
