...
if( ACTION_METHOD == 'hot' )
{
	...
	if( $album_id > 0 )
	{
		if( $mod == 'hot' and ($new_vid == 0 or $new_vid == 1) )
		{
			$sql = 'UPDATE ' . TABLE_PHOTO_NAME . '_album SET hot=' . $new_vid . ' WHERE album_id=' . $album_id;
			$db->query( $sql );
			$content = 'OK_' . $album_id;
		}
		$nv_Cache->delMod( $module_name );
...
if( ACTION_METHOD == 'add' || ACTION_METHOD == 'edit' )
{
	...
	foreach( $layout_array as $value )
	{
		...
		$xtpl->assign( 'LAYOUT', array(
			'key' => $value,
			'selected' => ($data['layout'] == $value) ? ' selected="selected"' : ''
		) );
	...
	foreach( $array_status as $key => $name )
	{
		$xtpl->assign( 'RATING', array(
			'key' => $key,
			'name' => $name,
			'selected' => ($key == $data['allow_rating']) ? 'selected="selected"' : ''
		) );
...
$base_url = NV_BASE_ADMINURL . 'index.php?' . NV_NAME_VARIABLE . '=' . $module_name . '&amp;' . NV_OP_VARIABLE . '=main&amp;sort=' . $sort . '&amp;order=' . $order . '&amp;per_page=' . $per_page;
...
foreach( $array_status as $key => $name )
{
	$xtpl->assign( 'STATUS', array(
		'key' => $key,
		'name' => $name,
		'selected' => ($key == $data['filter_status'] AND is_numeric( $data['filter_status'] )) ? 'selected="selected"' : ''
	) );
...
if( !empty( $array ) )
{
	foreach( $array as $item )
	{
		...
		$item['link_out'] =   $item['alias'] . '-' . $item['album_id'];
		...
		foreach( $array_status as $key => $name )
		{
			$xtpl->assign( 'HOT', array(
				'key' => $key,
				'name' => $name,
				'selected' => ($key == $item['hot']) ? 'selected="selected"' : ''
			) );
			$xtpl->parse( 'main.loop.hot' );
		}
