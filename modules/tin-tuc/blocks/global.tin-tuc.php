...
if (!nv_function_exists('nv_tintuc')) {
    /**
     * nv_block_config_tintuc()
     *
     * @param string $module
     ...
     * @return string
     */
    function nv_tintuc($block_config)
    {
        ...
        if (!empty($list)) {
            if (file_exists(NV_ROOTDIR . '/themes/' . $global_config['module_theme'] . '/modules/tin-tuc/block.tin-tuc.tpl')) {
                $block_theme = $global_config['module_theme'];
            } elseif (file_exists(NV_ROOTDIR . '/themes/' . $global_config['site_theme'] . '/modules/tin-tuc/block.tin-tuc.tpl')) {
                $block_theme = $global_config['site_theme'];
            } else {
                $block_theme = 'default';
            }

            $xtpl = new XTemplate('block.tin-tuc.tpl', NV_ROOTDIR . '/themes/' . $block_theme . '/modules/tin-tuc');

            foreach ($list as $l) {
				$l['image'] = NV_BASE_SITEURL . NV_FILES_DIR . '/' . $site_mods[$module]['module_upload'] . '/' . $l['image'];
                $l['description'] = nv_clean60($l['description'],120);
...
if (defined('NV_SYSTEM')) {
    $content = nv_tintuc($block_config);
}
