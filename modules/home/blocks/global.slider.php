...
if (!nv_function_exists('nvb_home_global_slider')) {
    /**
     * @param string $module
     ...
     * @return string
     */
    function nvb_config_home_global_slider($module, $data_block, $lang_block)
    {
     ...
     * @return number
     */
    function nvb_config_home_global_slider_submit($module, $lang_block)
    {
        global $nv_Request;
     ...
     * @return string
     */
    function nvb_home_global_slider($block_config)
    {
        ...
        if (isset($site_mods[$mod_name])) {
            ...
            $sql = 'SELECT title, title_extra, title_link, description, link, image, image_mobile, addtime, weight, status FROM ' . NV_PREFIXLANG . '_' . $mod_data . '_slider_rows where  status=1 AND  bid='.$block_config['blockid'].' AND status=1 order by weight ASC';
            $list = $nv_Cache->db($sql, '', $mod_name);

            if (file_exists(NV_ROOTDIR . '/themes/' . $global_config['module_theme'] . '/modules/' . $mod_file . '/global.slider.tpl')) {
                $block_theme = $global_config['module_theme'];
            } elseif (file_exists(NV_ROOTDIR . '/themes/' . $global_config['site_theme'] . '/modules/' . $mod_file . '/global.slider.tpl')) {
...
if (defined('NV_SYSTEM')) {
    $content = nvb_home_global_slider($block_config);
}
